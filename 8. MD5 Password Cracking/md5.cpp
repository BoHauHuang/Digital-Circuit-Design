...
void md5(uint8_t *initial_msg, size_t initial_len, uint8_t *hash)
{
    ...
    for (offset = 0; offset < pad_len; offset += (512/8))
    {
		...
		for(i = 0; i < 64; i++)
        {
            ...
            //printf("%08x %08x\n",w[g],LEFTROTATE((a + f + k[i] + w[g]), r[i]));
            a = temp;
...
int main(int argc, char **argv)
{
	...
	for (idx = 0; idx < 100000000; idx++) {
		sprintf((char*)pattern, "%08d", idx);
		md5(pattern, 8, hash1);
		if (!strncmp((char*)hash1, (char*)passwd_hash, 16)) break;
	}
